...
- Output → `models/loras/<culture>/dardesign-<culture>-lora.safetensors` (+ checkpoints at 500/1000/1500). The backend lazy-loads it from `models/loras/<culture>/` — no code change. **All three cultures are trained and deployed.** The cut order originally accepted Khaleeji/Moroccan (12–14 imgs) as prompt-only against Lebanese as the hero (19 imgs), and this note said so for a while after that stopped being true. Verified 2026-08-14: three files on disk, 93.1 MB / 1120 tensors each, three *distinct* sha256 — not copies of the Lebanese one — and a Colab render host loaded all three (`/healthz` → `light_mode: false`).
- **The weights are gitignored, so cloning the repo gets you code without models**, and every deployment target has needed its own delivery. There are three: Kaggle (`push_backend.py` attaches the `dardesign-train-lebanese` / `dardesign-train-rest` kernel outputs), and Google Drive for Colab — where the file has to be uploaded by hand, because Colab mounts My Drive only. A shared folder reaches Colab through a **shortcut** placed in My Drive; a same-named real folder next to that shortcut wins the name and hides it, which presents as `MISSING <culture>` while the weights sit there untouched. Colab caches the Drive tree, so `drive.mount(force_remount=True)` is needed after fixing it. Do not `mkdir` the project folder in the notebook — that is what creates the shadowing duplicate.
